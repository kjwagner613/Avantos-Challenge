...
const PrefillEditor = ({ formId }: { formId: string }) => {
    ...
    const getMapping = (fieldId: string) =>
        formMappings.find((m) => m.targetFieldId === fieldId)
    ...
    const clearMapping = (fieldId: string) => {
        setMappings((prev) => ({
            ...prev,
            [formId]: prev[formId]?.filter((m) => m.targetFieldId !== fieldId) ?? [],
        }))
    }
